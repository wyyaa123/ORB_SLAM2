...
namespace ORB_SLAM2
{
    ...
    void Frame::ExtractCurve(const cv::Mat &mImGray, const cv::Mat &depth, const CurveConfigPtr &curveConfigPtr, const cv::Mat &Sem)
    {
        ...
        mvBezierCurves.reserve(acceptedCurves.size());
    ...
    void Frame::assignProperty3DEach(orderedEdgePoint &pt, const cv::Mat &matDepth, const CurveConfigPtr &curveConfigPtr)
    {
        ...
        std::array<float, 25> validDepths; //-- 5x5 patch 最多包含25个有效深度
        ...
        for (int x_bias = -2; x_bias <= 2; ++x_bias)
        {
            for (int y_bias = -2; y_bias <= 2; ++y_bias)
            {
                ...
                if (depth > curveConfigPtr->minDepth && depth < curveConfigPtr->maxDepth)
                    validDepths[validDepthCount++] = depth;
            }
        }
        std::sort(validDepths.begin(), validDepths.begin() + validDepthCount);
        float adjusted_depth = 0; //-- 矫正后的深度

        //-- 计算前景深度
        if (validDepthCount >= 8)
        {
            //-- 检查跳变并返回第一组连续数据
            size_t foregroundCount = validDepthCount;
            const float rel_thres = 0.05f;
            for (size_t i = 1; i < validDepthCount; ++i)
            {
                float dx = validDepths[i] - validDepths[i - 1];
                float x = validDepths[i - 1];
            ...
            //-- 取最小的部分的深度的中位数作为深度值
            const float medianValue = (foregroundCount % 2 == 0)
                                          ? (validDepths[foregroundCount / 2 - 1] +
                                             validDepths[foregroundCount / 2]) * 0.5f
                                          : validDepths[foregroundCount / 2];
            if (depth_orig >= validDepths[0] &&
                depth_orig <= validDepths[foregroundCount - 1])
            {
        ...
        //-- 计算远近分数
        if (pt.depth > curveConfigPtr->minDepth && pt.depth < curveConfigPtr->maxDepth)
        {
            pt.x_3d = (pt.x - cx) * invfx * pt.depth;
            pt.y_3d = (pt.y - cy) * invfy * pt.depth;
            pt.z_3d = pt.depth;
        }
    }
    ...
    std::vector<cv::Point3d> Frame::UnprojectCurve(int i)
    {
        ...
        unprojectedPoints.reserve(curve.sampledPoints.size());

        const float r00 = mRwc.at<float>(0, 0);
        const float r01 = mRwc.at<float>(0, 1);
        const float r02 = mRwc.at<float>(0, 2);
        const float r10 = mRwc.at<float>(1, 0);
        const float r11 = mRwc.at<float>(1, 1);
        const float r12 = mRwc.at<float>(1, 2);
        const float r20 = mRwc.at<float>(2, 0);
        const float r21 = mRwc.at<float>(2, 1);
        const float r22 = mRwc.at<float>(2, 2);
        const float ox = mOw.at<float>(0);
        ...
        for (size_t i = 0; i < curve.sampledPoints.size(); ++i)
        {
            ...
            if (pt.depth > 0)
            {
                ...
                const float cameraY = static_cast<float>(pt.y_3d);
                const float cameraZ = static_cast<float>(pt.z_3d);
                unprojectedPoints.push_back(cv::Point3d(
                    r00 * cameraX + r01 * cameraY + r02 * cameraZ + ox,
                    r10 * cameraX + r11 * cameraY + r12 * cameraZ + oy,
                    r20 * cameraX + r21 * cameraY + r22 * cameraZ + oz));
            }
        }
