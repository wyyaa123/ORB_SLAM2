...
namespace ORB_SLAM2
{
    ...
    void LocalMapping::ProcessNewKeyFrameWithCurves()
    {
        ...
        set<MapCurve *> recentAddedMapCurves;
        ...
        for (size_t i = 0; i < vpMapCurveMatches.size(); i++)
        {
            ...
            if (pMC)
            {
                if (!pMC->isBad())
                {
                    ...
                    else if (pMC->mnFirstKFid == static_cast<long int>(mpCurrentKeyFrame->mnId) && recentAddedMapCurves.insert(pMC).second)
                    {
    ...
    void LocalMapping::SearchInNeighborsWithCurves()
    {
        ...
        vector<MapPoint *> vpMapPointMatches = mpCurrentKeyFrame->GetMapPointMatches();
        vector<MapCurve *> vpMapCurveMatches = mpCurrentKeyFrame->GetMapCurveMatches();
        for (vector<KeyFrame *>::iterator vit = vpTargetKFs.begin(), vend = vpTargetKFs.end(); vit != vend; vit++)
        {
            KeyFrame *pKFi = *vit;

            matcher.Fuse(pKFi, vpMapPointMatches);
            curveMatcher.Fuse(pKFi, vpMapCurveMatches);
            vpMapCurveMatches = mpCurrentKeyFrame->GetMapCurveMatches();
        }

        // Search matches by projection from target KFs in current KF
        vector<MapPoint *> vpFuseCandidates;
        vector<MapCurve *> vpCurveFuseCandidates;
        vpFuseCandidates.reserve(vpTargetKFs.size() * vpMapPointMatches.size());
        vpCurveFuseCandidates.reserve(vpTargetKFs.size() * vpMapCurveMatches.size());
        ...
        for (vector<KeyFrame *>::iterator vitKF = vpTargetKFs.begin(), vendKF = vpTargetKFs.end(); vitKF != vendKF; vitKF++)
        {
            ...
            vector<MapCurve *> vpMapCurvesKFi = pKFi->GetMapCurveMatches();

            for (vector<MapCurve *>::iterator vitMC = vpMapCurvesKFi.begin(), vendMC = vpMapCurvesKFi.end(); vitMC != vendMC; vitMC++)
            {
                MapCurve *pMC = *vitMC;
                if (!pMC)
                    continue;
                if (pMC->isBad() || pMC->mnFuseCandidateForKF == mpCurrentKeyFrame->mnId)
                    continue;
                pMC->mnFuseCandidateForKF = mpCurrentKeyFrame->mnId;
                vpCurveFuseCandidates.push_back(pMC);
            }
        }

        matcher.Fuse(mpCurrentKeyFrame, vpFuseCandidates);
        curveMatcher.Fuse(mpCurrentKeyFrame, vpCurveFuseCandidates);
